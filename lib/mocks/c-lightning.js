...
const debug = {
	info: require('debug')('lnurl-toolbox:mock:c-lightning:info'),
	error: require('debug')('lnurl-toolbox:mock:c-lightning:error'),
};
const fs = require('fs');
const { generateNodeKey, generatePaymentRequest, getTagDataFromPaymentRequest, SafeEventEmitter } = require('lnurl/lib');
...
module.exports = function(options, done) {
		...
		socket.on('data', data => {
			...
			_.chain(messages).map(message => {
				try {
					...
					app.interfaces.events.emit('jsonRpc:message', json);
					switch (json.method) {
						case 'fundchannel':
	...
	app.interfaces.events = new SafeEventEmitter();
	return app;
};
